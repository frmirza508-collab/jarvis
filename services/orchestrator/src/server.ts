...
const DEFAULT_ORIGINS = ['tauri://localhost', 'http://tauri.localhost', 'https://tauri.localhost', 'http://localhost:1420', 'http://127.0.0.1:1420'];
...
export async function createServer(opts: ServerOptions) {
  ...
  app.setErrorHandler((err, _req, reply) => {
    ...
    const status = (err as { statusCode?: number }).statusCode && !(err instanceof JarvisError) ? (err as { statusCode: number }).statusCode : (HTTP_STATUS[je.code] ?? 500);
  ...
  app.addHook('onRequest', async (req, reply) => {
    ...
    const presented = header.startsWith('Bearer ') ? header.slice(7) : req.url.startsWith('/events') ? (q ?? '') : '';
    if (!safeEqual(presented, opts.token)) return reply.status(401).send({ error: 'UNAUTHORIZED', message: 'Invalid session token' });
  });

  const persistSettings = () => {
    if (!opts.dataDir) return;
    mkdirSync(opts.dataDir, { recursive: true });
    const s: CoreSettings = { ...core.settings, roles: core.router.getRoles(), permissionPolicy: core.permissions.getPolicy() };
  ...
  app.get('/events', { websocket: true }, (socket) => {
    clients.add(socket);
    socket.send(JSON.stringify({ kind: 'hello', version: opts.version, pendingPermissions: [...core.permissions.pendingRequests()] }));
    socket.on('close', () => clients.delete(socket));
  });

  // --- requests ------------------------------------------------------------------
  const RequestBody = z.object({ text: z.string().min(1).max(20_000), language: z.string().optional(), sessionId: z.string().optional() });
  ...
  app.post<{ Params: { id: string } }>('/requests/:id/cancel', async (req) => ({ cancelled: core.orchestrator.cancel(req.params.id) }));
  app.get('/history', async () => core.memory.taskHistory(100));
  app.get<{ Querystring: { taskId?: string } }>('/events/history', async (req) => core.bus.getHistory({ taskId: req.query.taskId, limit: 500 }));
  ...
  app.post<{ Params: { id: string } }>('/permissions/:id/decision', async (req) => {
    const { decision } = z.object({ decision: z.enum(['allow_once', 'allow_always', 'deny']) }).parse(req.body);
  ...
  app.put('/permissions/policy', async (req) => {
    const p = z
      .object({
        ...
        rules: z.array(z.object({ id: z.string(), action: z.string(), targetPrefix: z.string().optional(), effect: z.enum(['allow', 'deny']), maxRisk: z.enum(['low', 'medium', 'high', 'critical']), createdAt: z.string() })),
      })
      .parse(req.body);
  ...
  app.get('/agents', async () =>
    core.registry.list().map((a) => ({ ...a.def, instructions: undefined, health: a.health, metrics: a.metrics, activeTasks: a.activeTasks })),
  );
  ...
  app.delete<{ Params: { id: string } }>('/agents/:id', async (req) => {
    if (!core.registry.unregister(req.params.id)) throw new JarvisError('INVALID_INPUT', 'Only dynamic agents can be removed; built-in agents can be disabled');
    return { ok: true };
  });
  app.get('/skills', async () => core.skills.list().map((s) => ({ id: s.id, name: s.name, description: s.description, category: s.category, tools: s.tools, usesModel: s.usesModel, stats: core.skills.getStats(s.id) })));
  app.get('/tools', async () => core.tools.list().map((t) => ({ id: t.id, title: t.title, description: t.description, module: t.module, categories: t.categories, status: core.tools.statusOf(t.id) })));
  ...
  app.get<{ Querystring: { q?: string; scope?: string } }>('/memory', async (req) => {
    ...
    return req.query.q ? core.memory.search(req.query.q, { scopes: scope ? [scope] : undefined, limit: 50 }) : core.memory.list({ scope, limit: 200 });
  });
  app.post('/memory', async (req) => {
    const b = z.object({ content: z.string().min(1).max(8000), scope: z.enum(MEMORY_SCOPES), scopeId: z.string().optional(), tags: z.array(z.string()).optional() }).parse(req.body);
    return core.memory.remember({ ...b, source: 'user', verified: true });
  });
  app.delete<{ Params: { id: string } }>('/memory/:id', async (req) => ({ ok: core.memory.forget(req.params.id) }));
  app.post<{ Params: { id: string } }>('/memory/:id/verify', async (req) => core.memory.verifyLesson(req.params.id, 'Verified by user'));
  app.post('/memory/purge', async (req) => {
    const b = z.object({ scope: z.enum(MEMORY_SCOPES).optional(), olderThanDays: z.number().int().min(0).optional(), expiredOnly: z.boolean().optional() }).parse(req.body);
  ...
  app.post<{ Params: { provider: string } }>('/providers/:provider/test', async (req) => {
    const p = core.router.getProvider(req.params.provider);
    if (!p) throw new JarvisError('NOT_FOUND', 'Unknown provider');
    const model = core.router.getRoles().fast.find((t) => t.provider === p.id)?.model ?? core.router.getRoles().reasoning[0]?.model;
    if (!model) throw new JarvisError('NOT_CONFIGURED', 'No model configured for this provider');
    const started = Date.now();
    const r = await p.chat({ model, messages: [{ role: 'user', content: 'Reply with the single word: ready' }], maxTokens: 5 });
    return { ok: true, model: r.model, latencyMs: Date.now() - started, reply: r.content };
  });
  app.get('/models/stats', async () => ({ stats: core.router.getStats(), log: core.router.getRequestLog(100) }));

  // --- settings ----------------------------------------------------------------------
  app.get('/settings', async () => ({ ...core.settings, roles: core.router.getRoles(), permissionPolicy: core.permissions.getPolicy(), languages: core.voiceLanguages.all() }));
  app.put('/settings/roles', async (req) => {
    const Target = z.object({ provider: z.string(), model: z.string().min(1) });
    const roles = z.object({ fast: z.array(Target), reasoning: z.array(Target), coding: z.array(Target), vision: z.array(Target), audio: z.array(Target), embedding: z.array(Target) }).parse(req.body);
  ...
  app.put<{ Params: { code: string } }>('/settings/languages/:code', async (req) => {
    const patch = z.object({ enabled: z.boolean().optional(), ttsVoice: z.string().optional() }).parse(req.body);
  ...
  app.post('/voice/transcribe', async (req) => {
    ...
    if (!Buffer.isBuffer(audio) || audio.length < 100) throw new JarvisError('INVALID_INPUT', 'Send raw audio with an audio/* content type');
    const hint = (req.headers['x-language-hint'] as string | undefined) || undefined;
    return core.speech.transcribe(audio, { mimeType: String(req.headers['content-type']), languageHint: hint });
  });
  app.post('/voice/speak', async (req, reply) => {
    const { text, language, voice } = z.object({ text: z.string().min(1).max(4000), language: z.string(), voice: z.string().optional() }).parse(req.body);
    if (!core.tts.isConfigured()) throw new JarvisError('NOT_CONFIGURED', 'Cloud TTS not configured; the desktop uses Windows voices');
