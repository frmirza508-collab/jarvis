...
export async function createUser(
  db: Queryable,
  input: { email: string; password: string; name?: string; role?: UserRow['role'] },
): Promise<UserRow> {
  if (input.password.length < 10)
    throw new JarvisError('INVALID_INPUT', 'Password must be at least 10 characters');
  const hash = await hashPassword(input.password);
  try {
    const r = await db.query<UserRow>(
      'INSERT INTO users (email, name, password_hash, role) VALUES ($1, $2, $3, $4) RETURNING id, email, name, role, disabled, created_at',
      [input.email.toLowerCase(), input.name ?? '', hash, input.role ?? 'customer'],
    );
    await audit(db, {
      actorType: 'system',
      action: 'user.create',
      ...
      details: { role: input.role ?? 'customer' },
    });
    return r.rows[0]!;
  } catch (e) {
    if ((e as { code?: string }).code === '23505')
      throw new JarvisError('INVALID_INPUT', 'An account with this email already exists');
    throw e;
  }
}
...
export async function login(
  ...
  const r = await db.query<UserRow & { password_hash: string }>('SELECT * FROM users WHERE email = $1', [
    email.toLowerCase(),
  ]);
  const u = r.rows[0];
  ...
  await db.query(
    `INSERT INTO sessions (user_id, token_hash, expires_at, ip, user_agent) VALUES ($1, $2, now() + interval '${SESSION_HOURS} hours', $3, $4)`,
    [u.id, sha256(token), meta.ip ?? null, meta.userAgent ?? null],
  );
  await audit(db, {
    actorType: u.role === 'customer' ? 'customer' : 'admin',
    actorId: u.id,
    action: 'auth.login',
    ip: meta.ip,
  });
