...
async function resolveUser(db: Db, e: BillingEvent): Promise<string | undefined> {
  ...
  if (e.subscriptionRef) {
    const r = await db.query<{ user_id: string }>(
      'SELECT user_id FROM subscriptions WHERE provider = $1 AND provider_ref = $2',
      [e.provider, e.subscriptionRef],
    );
  ...
  for (const e of events) {
    ...
    try {
      ...
      if (e.type === 'payment.succeeded' || e.type === 'subscription.renewed') {
        if (!userId || !e.planCode || e.amount === undefined || !e.currency)
          throw new Error('Event missing customer, plan or amount');
        await applyPayment(db, {
          userId,
          planCode: e.planCode,
          provider: e.provider,
          providerPaymentId: e.providerEventId,
          amount: e.amount,
          currency: e.currency,
          occurredAt: e.occurredAt,
          providerRef: e.subscriptionRef,
          actor: { type: 'provider', id: e.provider },
        });
      } else if (e.type === 'subscription.canceled' && userId) {
        await db.query(
          "UPDATE subscriptions SET status = 'canceled', updated_at = now() WHERE user_id = $1 AND status IN ('active', 'past_due')",
          [userId],
        );
        await audit(db, {
          actorType: 'provider',
        ...
        await audit(db, {
          actorType: 'provider',
          actorId: e.provider,
          action: `billing.${e.type}`,
          targetType: 'user',
          targetId: userId ?? null,
          details: { subscriptionRef: e.subscriptionRef },
        });
      }
      await db.query("UPDATE webhook_events SET status = 'processed', processed_at = now() WHERE id = $1", [
        id,
      ]);
      processed++;
    } catch (err) {
      ...
      await audit(db, {
        ...
        details: { eventId: e.providerEventId, error: (err as Error).message },
      });
    }
  }
