...
export async function buildApp(opts: AppOptions) {
    ...
    new StripeProvider(
      {
        ...
        priceMonthly: config.STRIPE_PRICE_MONTHLY,
        priceYearly: config.STRIPE_PRICE_YEARLY,
      },
      opts.fetchImpl,
    ),
    new SignedGatewayProvider({
      secret: config.GATEWAY_WEBHOOK_SECRET,
      checkoutUrl: config.GATEWAY_CHECKOUT_URL,
    }),
  ])
    providers.set(p.id, p);

  await app.register(cors, {
    origin: config.ADMIN_ORIGINS.split(',').map((s) => s.trim()),
    credentials: false,
  ...
  app.setErrorHandler((err, _req, reply) => {
    if (err instanceof LicenseError)
      return reply.status(err.httpStatus).send({ error: err.reason, message: err.message });
    if (err instanceof WebhookVerificationError)
      return reply.status(400).send({ error: 'invalid_webhook', message: err.message });
  ...
  const requireStaff =
    (roles: Array<UserRow['role']>) => async (req: FastifyRequest, reply: FastifyReply) => {
      const u = await userForToken(db, bearer(req));
      if (!u) return reply.status(401).send({ error: 'UNAUTHORIZED', message: 'Sign in required' });
  ...
  /** Update channel: desktop clients compare this with their version. */
  app.get('/v1/releases/latest', async () => (await getSettings(db)).latestRelease ?? { version: null });
  app.get(
    '/v1/plans',
    async () =>
      (
        await db.query(
          'SELECT code, name, amount, currency, interval, interval_count FROM plans WHERE active ORDER BY amount',
        )
      ).rows,
  );
  ...
  app.post('/v1/auth/login', strict, async (req) => {
    ...
    const { token, user } = await login(db, b.email, b.password, {
      ip: req.ip,
  ...
  app.get('/v1/me', { preHandler: requireUser }, async (req) => {
    ...
    const licenses = (
      await db.query(
        'SELECT id, key_last4, status, max_devices, created_at FROM licenses WHERE user_id = $1 ORDER BY created_at',
        [u.id],
      )
    ).rows;
    const devices = (
      await db.query(
        'SELECT d.id, d.name, d.platform, d.status, d.last_seen, d.license_id FROM devices d JOIN licenses l ON l.id = d.license_id WHERE l.user_id = $1 ORDER BY d.last_seen DESC',
        [u.id],
      )
    ).rows;
    const payments = (
      await db.query(
        'SELECT id, provider, plan_code, amount, currency, status, created_at FROM payments WHERE user_id = $1 ORDER BY created_at DESC LIMIT 50',
        [u.id],
      )
    ).rows;
    return {
  ...
  app.post('/v1/checkout', { preHandler: requireUser, ...strict }, async (req) => {
    ...
    if (!provider || !provider.isConfigured())
      throw new JarvisError('NOT_CONFIGURED', `Payment provider ${b.provider} is not configured`);
    const row = await getPlan(db, b.plan);
    const plan: Plan = {
      ...PLANS[b.plan],
      amount: row.amount,
      currency: row.currency as 'PKR',
      name: row.name,
    };
    const session = await provider.createCheckout({
      ...
      successUrl: b.successUrl ?? `${config.PUBLIC_BASE_URL}/checkout/success`,
      cancelUrl: b.cancelUrl ?? `${config.PUBLIC_BASE_URL}/checkout/cancel`,
    });
    await db.query(
      'INSERT INTO checkout_sessions (user_id, plan_code, provider, reference) VALUES ($1, $2, $3, $4)',
      [req.user!.id, b.plan, provider.id, session.reference],
    );
    await audit(db, {
      actorType: 'customer',
      actorId: req.user!.id,
      action: 'checkout.create',
      details: { plan: b.plan, provider: provider.id, reference: session.reference },
    });
    return session;
  });
  app.post('/v1/me/licenses', { preHandler: requireUser, ...strict }, async (req) => {
    const existing = (
      await db.query("SELECT COUNT(*)::int AS n FROM licenses WHERE user_id = $1 AND status = 'active'", [
        req.user!.id,
      ])
    ).rows[0].n as number;
    if (existing > 0)
      throw new JarvisError('INVALID_INPUT', 'You already have a license. Rotate its key if you lost it.');
    const { license, key } = await issueLicense(db, req.user!.id, { type: 'customer', id: req.user!.id });
    return { id: license.id, key };
  });
  app.post<{ Params: { id: string } }>(
    '/v1/me/licenses/:id/rotate',
    { preHandler: requireUser, ...strict },
    async (req) => {
      const own = await db.query('SELECT 1 FROM licenses WHERE id = $1 AND user_id = $2', [
        req.params.id,
        req.user!.id,
      ]);
  ...
  app.post<{ Params: { id: string } }>(
    '/v1/me/devices/:id/deactivate',
    { preHandler: requireUser },
    async (req) => {
      const own = await db.query(
        'SELECT 1 FROM devices d JOIN licenses l ON l.id = d.license_id WHERE d.id = $1 AND l.user_id = $2',
        [req.params.id, req.user!.id],
      );
      if (!own.rowCount) throw new JarvisError('NOT_FOUND', 'Device not found');
      return {
        ok: await deactivateDevice(db, { deviceId: req.params.id }, { type: 'customer', id: req.user!.id }),
      };
    },
  );
  ...
  app.post<{ Params: { provider: string } }>(
    ...
    async (req) => {
      ...
      if (!provider || !provider.isConfigured())
        throw new JarvisError('NOT_FOUND', 'Unknown webhook provider');
  ...
  app.get('/v1/admin/dashboard', { preHandler: staff }, async () => {
    ...
    const [subs, revenue, devices, customers, recent] = await Promise.all([
      ...
      q(
        "SELECT currency, SUM(amount)::bigint AS total, COUNT(*)::int AS n FROM payments WHERE status = 'succeeded' AND created_at > now() - interval '30 days' GROUP BY currency",
      ),
  ...
  app.post('/v1/admin/customers', { preHandler: admin }, async (req) => {
    const b = z
      .object({
        ...
        role: z.enum(['customer', 'support', 'admin']).default('customer'),
      })
      .parse(req.body);
    const u = await createUser(db, b);
    await audit(db, {
  ...
  app.get<{ Params: { id: string } }>('/v1/admin/customers/:id', { preHandler: staff }, async (req) => {
    ...
    const user = (
      await db.query('SELECT id, email, name, role, disabled, created_at FROM users WHERE id = $1', [id])
    ).rows[0];
    ...
    const [subscriptions, licenses, devices, payments, auditRows] = await Promise.all([
      ...
      db.query(
        'SELECT id, key_last4, status, max_devices, created_at, revoked_at, revoked_reason FROM licenses WHERE user_id = $1',
        [id],
      ),
      db.query(
        'SELECT d.* FROM devices d JOIN licenses l ON l.id = d.license_id WHERE l.user_id = $1 ORDER BY last_seen DESC',
        [id],
      ),
      db.query('SELECT * FROM payments WHERE user_id = $1 ORDER BY created_at DESC', [id]),
      db.query(
        "SELECT * FROM audit_events WHERE (target_type = 'user' AND target_id = $1) OR actor_id = $1 ORDER BY ts DESC LIMIT 100",
        [id],
      ),
    ]);
    return {
      ...
      devices: devices.rows.map(({ fingerprint: _f, ...d }) => d),
  ...
  app.post<{ Params: { id: string; action: string } }>(
    '/v1/admin/subscriptions/:id/:action',
    { preHandler: admin },
    async (req) => {
  ...
  app.post('/v1/admin/licenses', { preHandler: admin }, async (req) => {
    const b = z
      .object({ userId: z.string().uuid(), maxDevices: z.number().int().min(1).max(50).optional() })
      .parse(req.body);
  ...
  app.post<{ Params: { id: string; action: string } }>(
    ...
    async (req) => {
      ...
      const b = z
        ...
        .parse(req.body ?? {});
      if (action === 'rotate')
        return { key: await rotateLicenseKey(db, id, { type: 'admin', id: req.user!.id }) };
  ...
  app.post('/v1/admin/payments/manual', { preHandler: admin }, async (req) => {
    const b = z
      ...
      .parse(req.body);
    const { subscription, duplicate } = await applyPayment(db, {
      userId: b.userId,
      planCode: b.plan,
      provider: 'manual',
      providerPaymentId: b.reference,
      amount: b.amount,
      currency: b.currency,
      occurredAt: new Date(),
  ...
  app.get(
    ...
    async () =>
      (
        await db.query(
          'SELECT id, provider, provider_event_id, type, status, error, received_at, processed_at FROM webhook_events ORDER BY received_at DESC LIMIT 300',
        )
      ).rows,
  );

  app.get<{ Querystring: { action?: string; target?: string; limit?: string } }>(
    '/v1/admin/audit',
    { preHandler: staff },
    async (req) => {
      const r = await db.query(
        ...
        [
          req.query.action ? `%${req.query.action}%` : null,
  ...
  app.get(
    '/v1/admin/plans',
    { preHandler: staff },
    async () => (await db.query('SELECT * FROM plans ORDER BY amount')).rows,
  );
  app.put<{ Params: { code: string } }>('/v1/admin/plans/:code', { preHandler: admin }, async (req) => {
    ...
    const r = await db.query(
      'UPDATE plans SET name = $2, amount = $3, active = $4, updated_at = now() WHERE code = $1 RETURNING *',
      [cur.code, b.name ?? cur.name, b.amount ?? cur.amount, b.active ?? cur.active],
    );
    await audit(db, {
      actorType: 'admin',
      actorId: req.user!.id,
      ...
      targetType: 'plan',
      targetId: cur.code,
      details: { before: { amount: cur.amount, name: cur.name, active: cur.active }, after: b },
    });
    return r.rows[0];
  });

  app.get('/v1/admin/settings', { preHandler: staff }, async () => ({
    ...(await getSettings(db)),
    providers: [...providers.values()].map((p) => ({ id: p.id, configured: p.isConfigured() })),
  }));
    ...
    const b = z
      .object({
        graceHours: z
          ...
          .max(24 * 60)
          .optional(),
        canceledKeepsAccessUntilPeriodEnd: z.boolean().optional(),
        defaultMaxDevices: z.number().int().min(1).max(50).optional(),
        tokenTtlHours: z
          .number()
          .int()
          .min(1)
          .max(24 * 14)
          .optional(),
        bankTransferInstructions: z.string().max(4000).optional(),
        latestRelease: z
          .object({
