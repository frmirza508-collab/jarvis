      ...
      {data && (
        <>
          <div className="cards">
            ...
            <div className="card">
              <span>Suspended</span>
              <strong>{count('suspended')}</strong>
            </div>
            <div className="card">
              <span>Active devices</span>
              <strong>{data.activeDevices}</strong>
            </div>
            {data.revenue30d.map((r) => (
              <div className="card" key={r.currency}>
                <span>Revenue (30 days)</span>
                <strong>{fmtMoney(Number(r.total), r.currency)}</strong>
                <small>{r.n} payments</small>
              </div>
            ))}
          </div>
