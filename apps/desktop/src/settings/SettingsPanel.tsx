...
interface Settings {
  ...
  workspace: string;
  languages: Array<{ code: string; name: string; nativeName: string; enabled: boolean }>;
}
...
function Providers() {
  ...
  return (
    <div className="form">
      <p className="hint">
        Keys are encrypted with a key held in Windows Credential Manager. They are never shown again, never
        logged, and never passed to commands JARVIS runs.
      </p>
      {secrets.map((s) => (
        <div key={s.name} className="secret-row">
          <label htmlFor={s.name}>
            {SECRET_LABELS[s.name] ?? s.name}{' '}
            <StateBadge state={s.configured ? 'configured' : 'not_configured'} />
          </label>
          <div className="row">
            <input
              id={s.name}
              type="password"
              autoComplete="off"
              placeholder={s.configured ? '•••••••• (stored)' : 'Paste key'}
              value={values[s.name] ?? ''}
            ...
            {s.name === 'OPENROUTER_API_KEY' && s.configured && (
              <button
                onClick={async () => {
                  try {
                    const r = await core().post<{ model: string; latencyMs: number }>(
...
function Models() {
  ...
  useEffect(() => {
    ...
    void core()
      .get<Array<{ id: string; name: string }>>('/providers/openrouter/models')
      .then(setCatalog)
      .catch((e: Error) => setErr(`Model list unavailable: ${e.message}`));
  }, []);
  ...
  return (
    <div className="form">
      <p className="hint">
        Each role has an ordered fallback chain. JARVIS tries the next model when one fails or is
        rate-limited.
      </p>
      ...
      {(Object.keys(roles) as Array<keyof Roles>)
          ...
          <div key={role} className="role-row">
            ...
            {roles[role].map((t, i) => (
              <input
                ...
                list="models"
                value={t.model}
                onChange={(e) => update(role, i, e.target.value)}
                aria-label={`${role} model ${i + 1}`}
              />
            ))}
            <button
              onClick={() =>
                setSettings({
                  ...settings,
                  roles: { ...roles, [role]: [...roles[role], { provider: 'openrouter', model: '' }] },
                })
              }
            >
              + fallback
            ...
            {roles[role].length > 1 && (
              <button
                onClick={() =>
                  setSettings({ ...settings, roles: { ...roles, [role]: roles[role].slice(0, -1) } })
                }
              >
                −
              </button>
        ...
        onClick={async () => {
          try {
            const clean = Object.fromEntries(
              Object.entries(roles).map(([k, v]) => [k, v.filter((t) => t.model.trim())]),
            ) as Roles;
    ...
    <div className="form">
      <label className="check">
        <input
          ...
          onChange={(e) => setPrefs({ speakReplies: e.target.checked })}
        />{' '}
        Speak replies aloud
      </label>
      <label className="check">
        ...
        />{' '}
        Hands-free (start listening when I speak; otherwise click-to-talk)
      </label>
      <label>
      ...
      <table className="dense">
        <thead>
          <tr>
            <th>Language</th>
            <th>Enabled</th>
            <th>Windows voices installed</th>
            <th />
          </tr>
        </thead>
        <tbody>
          {settings?.languages.map((l) => {
            ...
            return (
              <tr key={l.code}>
                <td>
                  {l.name} <span dir="auto">{l.nativeName}</span>
                </td>
                <td>
                  <input
                    type="checkbox"
                    checked={l.enabled}
                    onChange={async (e) => {
                      await core().put(`/settings/languages/${l.code}`, { enabled: e.target.checked });
                      setSettings(await core().get<Settings>('/settings'));
                    }}
                  />
                </td>
                <td>
                  {vs.length ? (
                    ...
                    <span className="warn">
                      none — install via Windows Settings › Time &amp; language › Speech
                    </span>
                  )}
                </td>
                <td>
      ...
      <p className="hint">
        Additional languages are added through the language registry without bulk downloads; each language is
...
function Permissions() {
  ...
  return (
    <div className="form">
      <label>
        Always ask at or above risk
        <select
          value={policy.confirmAtOrAbove}
          onChange={(e) => void save({ ...policy, confirmAtOrAbove: e.target.value })}
        >
      ...
      <fieldset>
        ...
        {CATEGORIES.map((c) => (
          <label key={c} className="check">
            <input
              type="checkbox"
              checked={policy.autoAllow.includes(c)}
              onChange={(e) =>
                void save({
                  ...policy,
                  autoAllow: e.target.checked
                    ? [...policy.autoAllow, c]
                    : policy.autoAllow.filter((x) => x !== c),
                })
              }
            />{' '}
            {c}
        ...
        <p className="hint">
          DESTRUCTIVE actions (deleting files etc.) always ask. Blocked commands (disk formatting, disabling
          security, credential dumping) never run.
        </p>
      </fieldset>
      <h3>Saved rules</h3>
      <table className="dense">
        ...
        <tbody>
          {policy.rules.map((r) => (
            <tr key={r.id}>
              ...
              <td className="ellipsis">{r.targetPrefix ?? 'anywhere'}</td>
              <td>{r.effect}</td>
              <td>{r.maxRisk}</td>
              <td>
                <button
                  className="danger"
                  onClick={async () => {
                    await core().del(`/permissions/rules/${r.id}`);
                    load();
                  }}
                >
                  Remove
  ...
  return (
    <div className="form">
      <label>
        ...
        <select
          value={prefs.quality}
          onChange={(e) => setPrefs({ quality: e.target.value as typeof prefs.quality })}
        >
          <option value="high">High</option>
          <option value="medium">Medium</option>
          <option value="low">Low (weaker GPUs)</option>
        </select>
      </label>
      <label>
        Renderer (restart to apply)
        <select
      ...
      <p className="hint">
        Current renderer: {renderer ?? 'starting'}. Quality lowers automatically if the frame rate drops.
      </p>
      <label className="check">
        <input
          type="checkbox"
