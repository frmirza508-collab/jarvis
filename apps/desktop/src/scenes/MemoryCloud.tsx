...
const SCOPE_COLORS: Record<string, string> = { preference: '#ff7ad9', project: '#4f9dff', knowledge: '#5cffc8', lesson: '#ffe066', agent: '#c7a6ff', global: '#7fe3ff', session: '#56627f', task: '#9cff6b', department: '#ffb35c' };
...
export function MemoryCloud() {
  ...
  useEffect(() => {
    ...
    void core().get<MemoryItem[]>('/memory').then(setItems).catch(() => setItems([]));
  }, [view]);
  const positions = useMemo(() => {
    ...
    return items.map((it, i) => {
      ...
      return center.add(new THREE.Vector3(Math.cos(theta) * Math.sin(phi), Math.cos(phi), Math.sin(theta) * Math.sin(phi)).multiplyScalar(1.2));
    });
  }, [items]);
  useFrame((_, dt) => {
  ...
  return (
    <group position={MEMORY_CENTER}>
      <group ref={group}>
        {items.map((it, i) => (
          <mesh key={it.id} position={positions[i]} onPointerOver={(e) => { e.stopPropagation(); setHover(it); }} onPointerOut={() => setHover(null)}>
            <sphereGeometry args={[it.verified ? 0.16 : 0.1, 12, 12]} />
            <meshStandardMaterial color={SCOPE_COLORS[it.scope] ?? '#7fe3ff'} emissive={SCOPE_COLORS[it.scope] ?? '#7fe3ff'} emissiveIntensity={it.verified ? 2 : 0.8} />
          </mesh>
        ))}
        {hover && (
          <Html center distanceFactor={10} position={positions[items.indexOf(hover)]?.clone().add(new THREE.Vector3(0, 0.4, 0))} className="label3d">
      ...
      {items.length === 0 && view === 'memory' && (
        <Html center className="label3d">No memories stored yet</Html>
      )}
    </group>
  );
}
