...
const STATE_COLORS: Record<string, string> = { active: '#37c6ff', degraded: '#ffb547', not_configured: '#ffb547', unsupported: '#4a5070', disabled: '#4a5070', planned: '#4a5070' };
...
export function Modules3D({ showLabels }: { showLabels: boolean }) {
  ...
  return (
    <group>
      {MODULES.map((m, i) => (
        <ModuleStation key={m} id={m} position={modulePosition(i, MODULES.length)} state={moduleState(m, caps)} showLabel={showLabels} />
      ))}
    </group>
  );
}

function ModuleStation({ id, position, state, showLabel }: { id: string; position: THREE.Vector3; state: string; showLabel: boolean }) {
  ...
  return (
    <group position={position}>
      <group ref={ref} onClick={(e) => { e.stopPropagation(); go('modules'); }} onPointerOver={() => setHover(true)} onPointerOut={() => setHover(false)}>
        <mesh>
          <cylinderGeometry args={[0.55, 0.7, 0.18, 6]} />
          <meshStandardMaterial color={c} emissive={c} emissiveIntensity={state === 'active' ? 1.2 : 0.2} metalness={0.6} roughness={0.3} />
        </mesh>
        <mesh position={[0, 0.45, 0]}>
          <octahedronGeometry args={[0.22, 0]} />
          <meshStandardMaterial color={c} emissive={c} emissiveIntensity={state === 'active' ? 2 : 0.3} wireframe={state !== 'active'} />
        </mesh>
      </group>
      {(showLabel || hover) && (
