import { CoreClient, CoreError, resolveConnection } from './core-client';
import { applyLiveMessage, applyServerMessage, getState, notify, setState, takeEarly, type ViewId } from './store';
...
export async function connect(): Promise<void> {
  ...
  try {
    ...
    client = new CoreClient(conn);
    await client.get('/status');
    client.onMessage((m) => {
      const k = (m as { kind: string }).kind;
      if (k === 'graph' || k === 'delta') applyLiveMessage(m as unknown as Parameters<typeof applyLiveMessage>[0]);
      else applyServerMessage(m as ServerMessage);
    });
    client.onConnection((c) => setState({ connection: c ? 'connected' : 'disconnected' }));
    client.connect();
...
export async function sendRequest(text: string, language?: string): Promise<string | undefined> {
  ...
  try {
    const { requestId } = await core().post<{ requestId: string }>('/requests', { text: t, language, sessionId: sessionId() });
    setState((s) => ({
      messages: [...s.messages, { id: `m${++msgSeq}`, role: 'jarvis', text: '…', requestId, status: 'pending', ts: Date.now() }],
      live: { ...s.live, [requestId]: { id: requestId, text: t, nodes: {}, progress: [], agents: [], startedAt: Date.now() } },
    }));
    ...
    setState((s) => ({ messages: [...s.messages, { id: `m${++msgSeq}`, role: 'system', text: text2, status: 'error', ts: Date.now() }] }));
...
export async function decidePermission(id: string, decision: 'allow_once' | 'allow_always' | 'deny'): Promise<void> {
...
export function go(view: ViewId, extra: { agent?: string; department?: string } = {}): void {
  setState({ view, selectedAgent: extra.agent ?? getState().selectedAgent, selectedDepartment: extra.department ?? getState().selectedDepartment });
}
