...
import { EntitlementGuard, EntitlementSigner, EntitlementVerifier, generateSigningKeys, DEFAULT_GUARD_OPTIONS } from '../src/index.js';
...
async function setup(overrides: Record<string, unknown> = {}) {
  ...
  const token = await signer.sign(
    { sub: 'lic1', cid: 'c1', did: 'd1', fp: 'fp1', plan: 'monthly', status: 'active', ent: ['premium.execution'], entUntil: Math.floor(srvNow / 1000) + 30 * 86400, srvNow, nonce: 'n1', ...overrides },
    72 * 3600,
  );
...
describe('license tokens', () => {
  ...
  it('rejects tampered payloads, wrong device, replayed nonce and foreign keys', async () => {
    ...
    await expect(verifier.verify(forged, { fingerprint: 'fp1' })).rejects.toMatchObject({ reason: 'signature' });
    await expect(verifier.verify(token, { fingerprint: 'other' })).rejects.toMatchObject({ reason: 'device' });
    await expect(verifier.verify(token, { fingerprint: 'fp1', nonce: 'n2' })).rejects.toMatchObject({ reason: 'nonce' });
    const other = await setup();
    await expect(verifier.verify(other.token, { fingerprint: 'fp1' })).rejects.toMatchObject({ reason: 'signature' });
  });
});
...
describe('entitlement guard (clock tamper resistance)', () => {
  ...
  it('blocks after entitlement expiry even with a fresh token', async () => {
    const { verifier, token, srvNow } = await setup({ entUntil: Math.floor(Date.UTC(2026, 8, 1) / 1000) + 60 });
    const c = await verifier.verify(token, { fingerprint: 'fp1' });
    const g = new EntitlementGuard({ trustedTimeMs: 0, offlineElapsedMs: 0 });
    g.acceptOnline(token, c, srvNow);
