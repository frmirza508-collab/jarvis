...
describe('agent bus', () => {
  it('delivers typed events by type, task and recipient', async () => {
    ...
    bus.publish({ type: 'TASK_REQUEST', from: 'orchestrator', to: 'seo', taskId: 't1', payload: { taskId: 't1', goal: 'audit', requestedBy: 'u' } });
    bus.publish({ type: 'TASK_PROGRESS', from: 'seo', taskId: 't1', payload: { message: 'half', percent: 50 } });
  ...
  it('rejects unknown event types and waits for events', async () => {
    const bus = new AgentBus();
    expect(() => bus.publish({ type: 'NOPE' as 'BLOCKED', from: 'x', taskId: 't', payload: { reason: 'r' } })).toThrow();
    const p = bus.waitFor('COMPLETED', (e) => e.taskId === 'z', 1000);
    bus.publish({ type: 'COMPLETED', from: 'a', taskId: 'z', payload: { summary: 'done', artifacts: [], evidence: [] } });
