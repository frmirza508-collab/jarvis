...
export function documentTools(session: BrowserSession): ToolDefinition[] {
  return [
    defineTool({
      ...
      input: z.object({
        path: z.string().min(1),
        title: z.string().default('JARVIS Report'),
      ...
      execute: async (i, ctx) => {
        const p = path.resolve(
          ...
          i.path.endsWith('.pdf') ? i.path : `${i.path}.pdf`,
        );
        mkdirSync(path.dirname(p), { recursive: true });
        const { chromium } = await loadPlaywright();
        const browser = await chromium.launch({
          headless: true,
          executablePath: session.opts.executablePath,
          channel: session.opts.executablePath ? undefined : session.opts.channel,
        });
        try {
