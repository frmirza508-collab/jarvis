...
const mk = (confirm?: () => Promise<'allow_once' | 'allow_always' | 'deny'>) =>
  new PermissionManager(structuredClone(DEFAULT_POLICY), new AuditLog(), confirm);
...
describe('permission manager', () => {
  it('auto-allows low-risk READ without prompting', async () => {
    ...
    const pm = mk(confirm);
    expect(
      (await pm.request({ actor: 'a', action: 'fs.read', categories: ['READ'], description: 'read' }))
        .allowed,
    ).toBe(true);
    expect(confirm).not.toHaveBeenCalled();
  });
  it('denies protected actions when no UI handler is attached', async () => {
    const r = await mk().request({
      actor: 'a',
      action: 'shell.run',
      categories: ['EXECUTE'],
      description: 'x',
  ...
  it('asks the user and honours deny', async () => {
    ...
    expect(
      (await pm.request({ actor: 'a', action: 'fs.write', categories: ['WRITE'], description: 'x' })).allowed,
    ).toBe(false);
  });
  ...
  it('deny rules win', async () => {
    ...
    expect(
      (await pm.request({ actor: 'a', action: 'browser.open', categories: ['READ'], description: 'x' }))
        .allowed,
    ).toBe(false);
  });
});
