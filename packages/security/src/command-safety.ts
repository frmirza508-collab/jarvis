...
const RULES: Rule[] = [
  ...
  { re: /\brm\s+(-[a-z]*r[a-z]*f|-[a-z]*f[a-z]*r)[a-z]*\s+(\/|~|\*|\/\*)(\s|$)/i, risk: 'critical', reason: 'recursive delete of root/home', block: true },
  ...
  { re: /\bvssadmin\b.*\bdelete\s+shadows\b/i, risk: 'critical', reason: 'shadow copy deletion', block: true },
  ...
  { re: /\b(mimikatz|sekurlsa|lsadump)\b/i, risk: 'critical', reason: 'credential dumping tool', block: true },
  { re: /\breg(\.exe)?\s+save\s+hklm\\(sam|security|system)\b/i, risk: 'critical', reason: 'credential hive export', block: true },
  { re: /Set-MpPreference\b.*-Disable/i, risk: 'critical', reason: 'disabling Windows Defender', block: true },
  { re: /\bnetsh\s+advfirewall\s+set\s+\w+\s+state\s+off/i, risk: 'critical', reason: 'disabling firewall', block: true },
  ...
  { re: /\bgit\s+(push\s+.*--force|push\s+-f|reset\s+--hard|clean\s+-[a-z]*f)/i, risk: 'high', reason: 'destructive git operation' },
  { re: /\b(shutdown|restart-computer|stop-computer)\b/i, risk: 'high', reason: 'system power change' },
  { re: /\b(reg(\.exe)?\s+(add|delete)|Set-ItemProperty\s+.*HK(LM|CU))/i, risk: 'high', reason: 'registry modification' },
  { re: /\b(sc(\.exe)?\s+(delete|config|stop)|Stop-Service|Set-Service)\b/i, risk: 'high', reason: 'service modification' },
  ...
  { re: /(curl|wget|iwr|Invoke-WebRequest|irm|Invoke-RestMethod)\b.*\|\s*(sh|bash|iex|Invoke-Expression|powershell)/i, risk: 'high', reason: 'pipe remote script to shell' },
  ...
  { re: /\b(npm|pnpm|yarn|pip|winget|choco|cargo)\s+(install|add|i)\b/i, risk: 'medium', reason: 'package installation' },
  { re: /\b(curl|wget|Invoke-WebRequest|iwr)\b/i, risk: 'medium', reason: 'network download' },
  { re: /\b(mv|move|Move-Item|cp|copy|Copy-Item|xcopy|robocopy)\b/i, risk: 'medium', reason: 'file move/copy' },
