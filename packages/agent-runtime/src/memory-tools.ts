...
export function memoryTools(memory: MemoryStore): ToolDefinition[] {
  return [
    defineTool({
      ...
      execute: async (i) =>
        memory
          .search(i.query, { scopes: i.scopes, limit: i.limit })
          .map((m) => ({ id: m.id, scope: m.scope, content: m.content, verified: m.verified, tags: m.tags })),
    }),
    defineTool({
      id: 'memory.remember',
      title: 'Remember',
      description:
        'Store a durable fact or note for future tasks (project, knowledge or agent scope). Never store secrets.',
      module: 'memory',
      categories: ['WRITE'],
      input: z.object({
        content: z.string().min(3).max(4000),
        scope: z.enum(['project', 'knowledge', 'agent', 'department', 'global']),
