...
export const TEST_DATABASE_URL = process.env.TEST_DATABASE_URL ?? 'postgres://jarvis:jarvis@127.0.0.1:5432/jarvis_test';
...
export async function startLicenseServer(extra: Record<string, string> = {}, providers?: BillingProvider[]) {
  ...
  const config = loadConfig({ DATABASE_URL: TEST_DATABASE_URL, LICENSE_SIGNING_PRIVATE_KEY: keys.privateKeyPem, ...extra });
  const app = await buildApp({ db, config, providers });
  const address = await app.listen({ host: '127.0.0.1', port: 0 });
  await createUser(db, { email: 'admin@jarvis.test', password: 'admin-password-123', role: 'admin', name: 'Admin' });
  const call = async (method: string, url: string, body?: unknown, token?: string, headers: Record<string, string> = {}) => {
  ...
  const adminToken = ((await call('POST', '/v1/auth/login', { email: 'admin@jarvis.test', password: 'admin-password-123' })).body.token) as string;
  return { app, db, keys, address, call, adminToken, close: async () => { await app.close(); await db.end(); } };
}
