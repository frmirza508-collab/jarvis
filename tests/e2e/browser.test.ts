...
d('browser automation (real Chromium)', () => {
  ...
  for (const t of [...browserTools(session), ...documentTools(session), ...researchTools(new SearchRouter([])), pageMetaTool()]) rt.tools.register(t);
  ...
  it('navigates and extracts page content as untrusted data', async () => {
    const snap = (await rt.tools.invoke('browser.open', { url: site.url }, ctx)) as { title: string; text: string; forModel: string; links: unknown[] };
  ...
  it('runs a multi-step workflow: click, fill form, submit', async () => {
    ...
    const r = (await rt.tools.invoke('browser.fill_form', { fields: [{ label: 'Name', value: 'Fazal' }, { label: 'Email', value: 'f@example.com' }], submitSelector: '#send' }, ctx)) as { submitted: boolean };
  ...
  it('downloads files and takes screenshots', async () => {
    await rt.tools.invoke('browser.open', { url: site.url }, ctx);
    const d1 = (await rt.tools.invoke('browser.download', { text: 'Download price list' }, ctx)) as { path: string };
    expect(readFileSync(d1.path, 'utf8')).toContain('starter,100');
    const shot = (await rt.tools.invoke('browser.screenshot', { path: path.join(dl, 'shot.png') }, ctx)) as { path: string };
    expect(readFileSync(shot.path).subarray(1, 4).toString()).toBe('PNG');
  });

  it('flags prompt injection on hostile pages', async () => {
    const snap = (await rt.tools.invoke('browser.open', { url: `${site.url}/evil` }, ctx)) as { injection: { suspicious: boolean }; forModel: string };
    expect(snap.injection.suspicious).toBe(true);
    expect(snap.forModel).toContain('WARNING');
  });

  it('blocks non-http schemes', async () => {
    await expect(rt.tools.invoke('browser.open', { url: 'file:///etc/passwd' }, ctx)).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
  });

  it('creates a real PDF from Markdown', async () => {
    const r = (await rt.tools.invoke('documents.pdf', { path: 'report.pdf', title: 'Test', markdown: '# Report\n\n| a | b |\n|---|---|\n| 1 | 2 |\n\nاردو متن 中文' }, ctx)) as { path: string };
  ...
  it('extracts SEO metadata deterministically', async () => {
    ...
    expect(m).toMatchObject({ title: 'Acme Home', metaDescription: 'Acme builds rockets.', h1: ['Acme Rockets'], images: 2, imagesMissingAlt: 1, externalLinks: 1, lang: 'en' });
  });

  it('fetches pages over HTTP with untrusted wrapping', async () => {
    const p = (await rt.tools.invoke('web.fetch', { url: `${site.url}/evil` }, ctx)) as { injection: { suspicious: boolean } };
