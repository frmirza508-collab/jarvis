...
beforeAll(async () => {
  srv = await startLicenseServer({
    ...
    GATEWAY_WEBHOOK_SECRET: GATEWAY_SECRET,
  });
  userId = (
    await srv.call('POST', '/v1/auth/register', { email: 'hook@jarvis.test', password: 'customer-pass-123' })
  ).body.id as string;
});
  ...
  return JSON.stringify({
    ...
    data: {
      object: {
        ...
        metadata: { customer_id: userId, plan_code: 'monthly' },
      },
    },
  });
}
...
describe('payment webhooks', () => {
  it('rejects unsigned and wrongly signed Stripe webhooks without side effects', async () => {
    ...
    expect(
      (
        await srv.call('POST', '/v1/webhooks/stripe', body, undefined, {
          'Stripe-Signature': stripeSig(body, undefined, 'whsec_wrong'),
        })
      ).status,
    ).toBe(400);
    ...
    expect(
      (
        await srv.call('POST', '/v1/webhooks/stripe', body, undefined, {
          'Stripe-Signature': stripeSig(body, stale),
        })
      ).status,
    ).toBe(400);
  ...
  it('activates a subscription from a verified Stripe webhook exactly once', async () => {
    ...
    const r1 = await srv.call('POST', '/v1/webhooks/stripe', body, undefined, {
      'Stripe-Signature': stripeSig(body),
    });
    expect(r1.status).toBe(200);
    expect(r1.body.status).toBe('processed');
    const r2 = await srv.call('POST', '/v1/webhooks/stripe', body, undefined, {
      'Stripe-Signature': stripeSig(body),
    });
  ...
  it('records failed processing for an underpaid webhook', async () => {
    ...
    const r = await srv.call('POST', '/v1/webhooks/stripe', body, undefined, {
      'Stripe-Signature': stripeSig(body),
    });
    expect(r.status).toBe(200);
    const ev = await srv.db.query(
      "SELECT status, error FROM webhook_events WHERE provider_event_id = 'evt_under'",
    );
  ...
  it('accepts HMAC-signed gateway events and rejects tampering', async () => {
    ...
    const bad = await srv.call('POST', '/v1/webhooks/gateway', body.replace('40000', '50000'), undefined, {
      'X-Jarvis-Timestamp': String(ts),
      'X-Jarvis-Signature': sig,
    });
    expect(bad.status).toBe(400);
    const ok = await srv.call('POST', '/v1/webhooks/gateway', body, undefined, {
      'X-Jarvis-Timestamp': String(ts),
      'X-Jarvis-Signature': sig,
    });
