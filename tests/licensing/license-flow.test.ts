...
async function newCustomer(email: string) {
  const reg = await srv.call('POST', '/v1/auth/register', { email, password: 'customer-pass-123', name: 'Customer' });
...
function desktop(fp: string, secrets = new MemorySecretStore()) {
  return new LicenseClient({ apiUrl: srv.address, publicKeyPem: srv.keys.publicKeyPem, secrets, appVersion: 'test', fingerprint: fp.padEnd(64, '0') });
}
...
describe('licensing end-to-end (real Postgres + real desktop client)', () => {
  it('monthly plan: pay -> license -> activate -> premium', async () => {
    ...
    const pay = await srv.call('POST', '/v1/admin/payments/manual', { userId: c.id, plan: 'monthly', amount: 4000, reference: 'BANK-001' }, srv.adminToken);
    expect(pay.status).toBe(200);
    const sub = pay.body.subscription as { status: string; current_period_end: string; current_period_start: string };
    ...
    const dup = await srv.call('POST', '/v1/admin/payments/manual', { userId: c.id, plan: 'monthly', amount: 4000, reference: 'BANK-001' }, srv.adminToken);
    expect(dup.body.duplicate).toBe(true);
  });

  it('rejects underpayment', async () => {
    const c = await newCustomer('under@jarvis.test');
    const r = await srv.call('POST', '/v1/admin/payments/manual', { userId: c.id, plan: 'yearly', amount: 4000, reference: 'BANK-UNDER' }, srv.adminToken);
    expect(r.status).toBe(400);
  });

  it('yearly plan grants ~1 year', async () => {
    const c = await newCustomer('yearly@jarvis.test');
    const pay = await srv.call('POST', '/v1/admin/payments/manual', { userId: c.id, plan: 'yearly', amount: 40000, reference: 'BANK-Y1' }, srv.adminToken);
  ...
  it('expiry stops premium; payment reactivates', async () => {
    ...
    await srv.call('POST', '/v1/admin/payments/manual', { userId: c.id, plan: 'monthly', amount: 4000, reference: 'BANK-E1' }, srv.adminToken);
    const lic = await srv.call('POST', '/v1/me/licenses', {}, c.token);
    ...
    await srv.db.query("UPDATE subscriptions SET current_period_end = now() - interval '1 minute' WHERE user_id = $1", [c.id]);
    ...
    await srv.call('POST', '/v1/admin/payments/manual', { userId: c.id, plan: 'monthly', amount: 4000, reference: 'BANK-E2' }, srv.adminToken);
  ...
  it('grace period keeps past_due access, then expires', async () => {
    ...
    await srv.call('POST', '/v1/admin/payments/manual', { userId: c.id, plan: 'monthly', amount: 4000, reference: 'BANK-G1' }, srv.adminToken);
    const lic = await srv.call('POST', '/v1/me/licenses', {}, c.token);
    const client = desktop('dd');
    await client.init();
    await client.activate(lic.body.key as string);
    await srv.db.query("UPDATE subscriptions SET current_period_end = now() - interval '1 hour' WHERE user_id = $1", [c.id]);
    await reconcileAll(srv.db);
    expect(await client.refresh()).toMatchObject({ premium: true, status: 'past_due' });
    await srv.db.query("UPDATE subscriptions SET current_period_end = now() - interval '49 hours' WHERE user_id = $1", [c.id]);
    await reconcileAll(srv.db);
  ...
  it('enforces device limits and allows deactivation', async () => {
    ...
    await srv.call('POST', '/v1/admin/payments/manual', { userId: c.id, plan: 'monthly', amount: 4000, reference: 'BANK-D1' }, srv.adminToken);
  ...
  it('admin revocation blocks the device immediately on next check', async () => {
    ...
    await srv.call('POST', '/v1/admin/payments/manual', { userId: c.id, plan: 'yearly', amount: 40000, reference: 'BANK-R1' }, srv.adminToken);
    const lic = await srv.call('POST', '/v1/me/licenses', {}, c.token);
    const client = desktop('ee');
    await client.init();
    await client.activate(lic.body.key as string);
    const licId = lic.body.id as string;
    const rev = await srv.call('POST', `/v1/admin/licenses/${licId}/revoke`, { reason: 'chargeback' }, srv.adminToken);
  ...
  it('suspension blocks and reactivation restores', async () => {
    ...
    const pay = await srv.call('POST', '/v1/admin/payments/manual', { userId: c.id, plan: 'monthly', amount: 4000, reference: 'BANK-S1' }, srv.adminToken);
    ...
    expect((await client.refresh()).premium).toBe(true);
    const ext = await srv.call('POST', `/v1/admin/subscriptions/${subId}/extend`, { days: 10, reason: 'goodwill' }, srv.adminToken);
  ...
  it('rejects forged tokens from a server with a different key', async () => {
    ...
    await srv.call('POST', '/v1/admin/payments/manual', { userId: c.id, plan: 'monthly', amount: 4000, reference: 'BANK-F1' }, srv.adminToken);
    const key = (await srv.call('POST', '/v1/me/licenses', {}, c.token)).body.key as string;
    const client = new LicenseClient({ apiUrl: srv.address, publicKeyPem: other.publicKeyPem, secrets: new MemorySecretStore(), appVersion: 't', fingerprint: 'ab'.padEnd(64, '0') });
    await client.init();
  ...
  it('rejects malformed license requests and invalid keys', async () => {
    expect((await srv.call('POST', '/v1/licenses/activate', { licenseKey: 'nope', fingerprint: 'x', nonce: 'y' })).status).toBe(400);
