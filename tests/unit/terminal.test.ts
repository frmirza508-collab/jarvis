...
describe('terminal tools', () => {
  it('runs commands after permission and returns output', async () => {
    ...
    const r = (await rt.tools.invoke('shell.run', { command: process.platform === 'win32' ? 'Write-Output hi' : 'echo hi' }, { actor: 'test', workspace: os.tmpdir() })) as { exitCode: number; stdout: string };
  ...
  it('never runs blocked commands', async () => {
    ...
    const rt = makeRuntime(confirm);
    for (const t of terminalTools()) rt.tools.register(t);
    await expect(rt.tools.invoke('shell.run', { command: 'rm -rf /' }, { actor: 'test' })).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
  ...
  it('scrubs JARVIS secrets from child environment', async () => {
    ...
    for (const t of terminalTools()) rt.tools.register(t);
    const r = (await rt.tools.invoke('shell.run', { command: process.platform === 'win32' ? 'Write-Output $env:OPENROUTER_API_KEY' : 'echo "k=$OPENROUTER_API_KEY"' }, { actor: 't' })) as { stdout: string };
